...
void Class_Chassis_Steering::Init()
{
   ...
   for (int i = 0; i < 4; i++)
    ...
    for (int i = 0; i < 4; i++)
    {
        Steering_Motor[i].Init(&hfdcan1, Steering_Motor_Id_Enum[i], Motor_DJI_Control_Method_ANGLE, 19.0f);
    }
...
void Class_Chassis_Steering::CAN_RxCallback(uint32_t stdId, uint8_t* data)
{
    for (int i = 0; i < 4; i++)
    {
        ...
        {
            Steering_Motor[i].CAN_RxCpltCallback();
            return;
        }
    }
}
...
void Class_Chassis_Steering::_Steer_Motor_Kinematics_Nearest_Transposition()
{
    ...
    for (int i = 0; i < 4; i++)
    {
        float tmp_delta_angle = Basic_Math_Modulus_Normalization(Target_Steer_Angle[i] - Steering_Motor[i].Get_Now_Angle(), 2.0f * PI);
        ...
        {
            // 需要反转扣圈情况
            Target_Steer_Angle[i] = Basic_Math_Modulus_Normalization(tmp_delta_angle + PI, 2.0f * PI) + Steering_Motor[i].Get_Now_Angle();
