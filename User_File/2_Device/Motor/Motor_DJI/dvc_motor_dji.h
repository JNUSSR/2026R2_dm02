...
public:
    ...
    inline float Get_Out() const;   //A板的驱动中有该接口，但新版中没有，为适配一些模块的接口添加

    inline void Set_Out(const float &__Out);//A板的驱动中有该接口，但新版中没有，为适配一些模块的接口添加
 ...
 * @param __Out 输出量
 */
inline void Class_Motor_DJI_GM6020::Set_Out(const float &__Out)
{
    Out = __Out;
}
 ...
 * @param __Out 输出量
 */
inline void Class_Motor_DJI_C610::Set_Out(const float &__Out)
{
    Out = __Out;
}
 ...
 * @param __Out 输出量
 */
inline void Class_Motor_DJI_C620::Set_Out(const float &__Out)
{
    Out = __Out;
}
