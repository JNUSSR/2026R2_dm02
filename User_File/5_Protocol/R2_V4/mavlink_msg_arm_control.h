...
typedef struct __mavlink_arm_control_t {
 uint8_t target_arm; /*<  Which arm to execute the command*/
 uint8_t action_cmd; /*<  Command ID to execute*/
} mavlink_arm_control_t;

#define MAVLINK_MSG_ID_ARM_CONTROL_LEN 2
#define MAVLINK_MSG_ID_ARM_CONTROL_MIN_LEN 2
#define MAVLINK_MSG_ID_200_LEN 2
#define MAVLINK_MSG_ID_200_MIN_LEN 2

#define MAVLINK_MSG_ID_ARM_CONTROL_CRC 235
#define MAVLINK_MSG_ID_200_CRC 235
...
#define MAVLINK_MESSAGE_INFO_ARM_CONTROL { \
    ...
    "ARM_CONTROL", \
    2, \
    {  { "target_arm", NULL, MAVLINK_TYPE_UINT8_T, 0, 0, offsetof(mavlink_arm_control_t, target_arm) }, \
         { "action_cmd", NULL, MAVLINK_TYPE_UINT8_T, 0, 1, offsetof(mavlink_arm_control_t, action_cmd) }, \
         } \
}
#else
#define MAVLINK_MESSAGE_INFO_ARM_CONTROL { \
 ...
 *
 * @param target_arm  Which arm to execute the command
...
{
    return mavlink_msg_arm_control_pack(system_id, component_id, msg, arm_control->target_arm, arm_control->action_cmd);
}
...
{
    return mavlink_msg_arm_control_pack_chan(system_id, component_id, chan, msg, arm_control->target_arm, arm_control->action_cmd);
}
...
{
    return mavlink_msg_arm_control_pack_status(system_id, component_id, _status, msg,  arm_control->target_arm, arm_control->action_cmd);
}
...
static inline void mavlink_msg_arm_control_send(mavlink_channel_t chan, uint8_t target_arm, uint8_t action_cmd)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
    char buf[MAVLINK_MSG_ID_ARM_CONTROL_LEN];
    _mav_put_uint8_t(buf, 0, target_arm);
...
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
    mavlink_msg_arm_control_send(chan, arm_control->target_arm, arm_control->action_cmd);
#else
 ...
 */
static inline void mavlink_msg_arm_control_send_buf(mavlink_message_t *msgbuf, mavlink_channel_t chan,  uint8_t target_arm, uint8_t action_cmd)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
...
/**
 * @brief Get field target_arm from arm_control message
 *
 * @return  Which arm to execute the command
 */
static inline uint8_t mavlink_msg_arm_control_get_target_arm(const mavlink_message_t* msg)
{
...
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
    arm_control->target_arm = mavlink_msg_arm_control_get_target_arm(msg);
